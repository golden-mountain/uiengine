...
import { IDataPoolHandle } from '../../typings'
  ...
  describe('Test set and get function:', () => {
    ...
    it('should set and get the data info successfully:', () => {
      ...
      const firstSet = dataPool.set(
        '#a.b:',
        'this is root node',
        { dataInfo: { key: 'status', value: 'create' } }
      )
      expect(firstSet).to.equal(true)
      let wholeData = dataPool.get()
      expect(wholeData).to.deep.equal(
      ...
      const secondSet = dataPool.set('a.b:c1', 'this is c1 node', { dataInfo: { key: 'status', value: 'create' } })
      expect(secondSet).to.equal(true)
      const thirdSet = dataPool.set('a.b:c2', 'this is c2 node', { dataInfo: { key: 'status', value: 'update' } })
      expect(thirdSet).to.equal(true)
      const forthSet = dataPool.set('a.b:c3', 'this is c3 node', { dataInfo: { key: 'status', value: 'delete' } })
      expect(forthSet).to.equal(true)
      const fifthSet = dataPool.set('a.b:c4', 'this is c4 node', { dataInfo: { key: 'status', value: 'view' } })
      ...
      const sixthSet = dataPool.set(
        ...
        { dataInfo: { key: 'status', defaultValue: 'create' } }
      )
      expect(sixthSet).to.equal(true)
      ...
      const seventhSet = dataPool.set(
        ...
        { dataInfo: { key: 'status', value: 'update' } }
      )
      ...
      const eighthSet = dataPool.set(
        ...
        },
        {
          dataInfo: [
            {
              key: 'type',
              setDataInfo: (infoKey: string, handle: IDataPoolHandle) => {
                const data = handle.getData()
                if (typeof data === 'object') {
                  handle.setInfo('type', 'branch node')
                }
                if (typeof data === 'string') {
                  handle.setInfo('type', 'leaf node')
                }
              }
            },
            {
              key: 'route',
              setDataInfo: (infoKey: string, handle: IDataPoolHandle) => {
                const route = handle.getRoute()
                handle.setInfo('route', route)
              }
            }
          ]
        }
      ...
      info = dataPool.get('a.b:c.d', { content: 'route' })
      expect(info).to.equal('a.b.c.d')
      info = dataPool.get('a.b:c.d.d1', { content: 'route' })
      expect(info).to.equal('a.b.c.d.d1')
      info = dataPool.get('a.b:c.d.d2', { content: 'route' })
      expect(info).to.equal('a.b.c.d.d2')
  ...
  describe('Test transfer function:', () => {
    it('should transfer the data value successfully:', () => {
      ...
      const firstSet = dataPool.set(
        ...
        { dataInfo: { key: 'status', value: 'view' } }
      )
      expect(firstSet).to.equal(true)
      let wholeData = dataPool.get()
      expect(wholeData).to.deep.equal(
    ...
    it('should transfer the data info successfully:', () => {
      ...
      const secondSet = dataPool.set(
        'a.b:c.c5',
        'this is c5 node of c',
        { dataInfo: { key: 'type', value: 'string' } }
      )
      expect(secondSet).to.equal(true)
      ...
      const thirdSet = dataPool.set(
        'a.b:e.c6',
        123456,
        { dataInfo: { key: 'type', value: 'number' } }
      )
      expect(thirdSet).to.equal(true)
        ...
        },
        {
          dataInfo: {
            ...
            setDataInfo: (infoKey: string, handle: IDataPoolHandle) => {
              const data = handle.getData()
              if (typeof data === 'object') {
                handle.setInfo('type', 'object')
              } else if (typeof data === 'string') {
                handle.setInfo('type', 'string')
              } else {
                handle.setInfo('type', 'undefined')
              }
            }
          }
        }
