...
export class PluginManager implements TYPES.IPluginManager {
  ...
  private getPluginWeight(plugin: TYPES.IPlugin, scopePath?: string) {
    ...
    pluginArray.forEach((plugin: TYPES.IPlugin) => {
      ...
      if (_.isArray(categories) && categories.length > 0) {
        categoryList = categories.map(
          (config: string | TYPES.IPluginCategoryConfig) => {
      ...
      if (_.isArray(scopePaths) && scopePaths.length > 0) {
        workingScopes = scopePaths.map(
          (config: string | TYPES.IPluginScopeConfig) => {
            if (!_.isString(config)) {
        ...
        if (!_.isNil(scopeConfig)) {
          ...
          categoryList.forEach((category: string) => {
            ...
            if (_.isNil(categoryMap[category][name])) {
              ...
              const result = resolver(prevPlugin, nextPlugin, {
                scopePath,
                category
              });
              if (!_.isEmpty(result)) {
  ...
  unloadPlugins(
    ...
    if (_.isString(scopePath) && scopePath.length > 0) {
      const scopeConfig = this.searchPluginScope(
        this.scopes,
        scopePath.split(".")
      );
      if (!_.isNil(scopeConfig)) {
  ...
  getPlugins(scopePath?: string, category?: string, name?: string) {
  ...
  register(id: string, info?: TYPES.IPluginCallerRegisterInfo) {
    ...
    if (!_.isNil(id) && _.isString(id) && !_.isEmpty(id)) {
      ...
      if (!_.isNil(node)) {
        if (
          ...
          if (!_.isNil(subNode)) {
            return this.filterHistoryRecords(
              this.mapHistoryRecords(subNode.indexes),
              exclude
            );
          }
        } else {
          return this.filterHistoryRecords(
            this.mapHistoryRecords(node.indexes),
            exclude
          );
        }
      }
  ...
  exportHistoryRecords(options?: TYPES.IPluginExportOption) {
    ...
    let exportHistory:
      | TYPES.IPluginExportTree
      | TYPES.IPluginExecuteRecord[] = {};
      ...
      if (_.isNil(validCategory) || !validCategory.includes(category)) {
        ...
        if (_.isArray(workScope) && workScope.length > 0) {
          workScope.forEach((scopeString: string) => {
            ...
            let currentMap: { [key: string]: TYPES.IPluginScope } | null = this
              .scopes;
        ...
        if (!_.isNil(options)) {
          ...
          // sort by priority
          const midQueue: TYPES.IPlugin[] = this.sortPluginsByPriority(
            pluginMap,
            category
          );
          executeQueue = executeQueue.concat(beforeQueue, midQueue, afterQueue);
            ...
            try {
              ...
              if (_.isArray(result)) {
                ...
                if (validPlugin) {
                  ...
                  throw Error(
                    "Plugin invoker returns invalid plugins which has no name"
                  );
                }
              }
            } catch (e) {
  ...
  private adapteParam(param: any, plugin: TYPES.IPlugin, category: string) {
    if (_.isObject(plugin) && _.isObject(param)) {
      const { categories } = plugin;
      if (_.isArray(categories)) {
        let adapter:
          | TYPES.IPluginParamRouteMap
          | TYPES.IPluginParamAdapter
          | any = null;
        categories.forEach((config: string | TYPES.IPluginCategoryConfig) => {
          if (_.isObject(config) && config.name === category) {
  ...
  async executePlugin(id: string, plugin: TYPES.IPlugin, param: any) {
  ...
  async executePlugins(
    ...
    if (!_.isNil(options)) {
      ...
      for (let index = 0; index < executeQueue.length; index++) {
        ...
        // execute
        records.push(
          await this.asyncExecute(plugin, realParam, {
            caller: id,
            category,
            queue: executeQueue,
            records
  ...
  syncExecutePlugin(id: string, plugin: TYPES.IPlugin, param: any) {
    const startNumber = ++this.history.lastStartNumber;
    ...
    if (!_.isNil(options)) {
      ...
      for (let index = 0; index < executeQueue.length; index++) {
        ...
        if (realParam instanceof Promise) {
          console.log(
            `Error: The adapter returns a Promise param in sync execution.(id:${id}, category:${category}, plugin:${plugin.name})`
          );
        }
      ...
      for (let index = 0; index < executeQueue.length; index++) {
        ...
        // execute
        records.push(
          this.syncExecute(plugin, realParam, {
            caller: id,
            category,
            queue: executeQueue,
            records
