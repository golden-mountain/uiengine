...
export class ListenerManager implements TYPES.IListenerManager {
    ...
    listenerArray.forEach((config: TYPES.IListenerConfig) => {
      if (_.isObject(config)) {
        ...
        if (_.isNil(this.listeners[name])) {
          ...
          const result = resolver(prevListener, nextListener);
          if (!_.isEmpty(result)) {
  ...
  private prepareListenerQueue(eventConfig: TYPES.IEventConfig) {
    ...
    if (_.isString(listener)) {
      if (!_.isNil(this.listeners[listener])) {
        listenerQueue.push(_.cloneDeep(this.listeners[listener]));
      }
      ...
      listener.forEach((config: string | TYPES.IEventListenerConfig) => {
        if (_.isString(config)) {
          if (!_.isNil(this.listeners[config])) {
            listenerQueue.push(_.cloneDeep(this.listeners[config]));
          }
  ...
  private generateDebugInfo(
    dataSource: any,
    debugList: Array<
      string | TYPES.IEventDebugConfig | TYPES.IListenerDebugConfig
    >
  ) {
    ...
    if (_.isArray(debugList) && debugList.length > 0) {
      debugList.forEach(
        (
          config: string | TYPES.IEventDebugConfig | TYPES.IListenerDebugConfig
        ) => {
          if (_.isString(config)) {
  ...
  private callEventListener(
    ...
    if (_.isObject(listenerConfig)) {
      ...
      let directParam: TYPES.IListenerParam = {};
      ...
      if (_.isFunction(adapter)) {
        ...
        try {
          ...
          console.error(
            `Error happens in the param adapter for listener "${name}".`
          );
          console.error(
            "Which is called in the event ",
      ...
      if (_.isArray(paramKeys) && paramKeys.length > 0) {
        ...
        directParam = adaptedParam;
      }

      if (_.isArray(debugList) && debugList.length > 0) {
        listenerRecord.originInfo = this.generateDebugInfo(
          directParam,
          debugList
        );
      }
        ...
        try {
          ...
          if (result instanceof Promise) {
            listenerRecord.result = result.then((returnData: any) => {
              if (_.isArray(debugList) && debugList.length > 0) {
                listenerRecord.finialInfo = this.generateDebugInfo(
                  directParam,
                  debugList
                );
              }
  ...
  private defaultEventListener(
    ...
    console.log(
      `In the "${eventName}" event, the "${listenerName}" listener has no function to call.`
    );
    if (target) {
      console.log(`Target is: `, target);
    }
  }
  private storeHistoryRecord(eventRecord: TYPES.IEventRecord) {
    ...
    if (_.isString(targetName) && targetName) {
      ...
      const { indexes: tIndexes, eventTree: subEventTree } = targetTree[
        targetName
      ];
      ...
      if (_.isString(eventName) && eventName) {
        ...
        const { indexes: eIndexes, targetTree: subTargetTree } = eventTree[
          eventName
        ];
      ...
      const hasStored = Object.keys(targetTree).some((currentName: string) => {
        const {
          reference,
          indexes: tIndexes,
          eventTree: subEventTree
        } = targetTree[currentName];
    ...
    if (_.isArray(listenerRecords) && listenerRecords.length > 0) {
      listenerRecords.forEach(
        (record: TYPES.IListenerRecord, index: number) => {
      ...
      if (_.isObject(config)) {
        ...
        if (
          ...
          if (_.isString(eventName) && eventName.length > 0) {
            ...
            // save param config
            const defaultConfig = _.isObject(defaultParams)
              ? { ...defaultParams }
              : {};
          ...
          if (_.isString(eventName) && eventName.length > 0) {
            ...
            const listenerQueue = this.prepareListenerQueue(config);
            // save param config
            const receiveConfig = _.isArray(receiveParams)
              ? [...receiveParams]
              : [];
            const defaultConfig = _.isObject(defaultParams)
              ? { ...defaultParams }
            ...
            props[eventName] = (...args: any[]) => {
              ...
              if (_.isArray(debugConfig) && debugConfig.length > 0) {
                eventRecord.originInfo = this.generateDebugInfo(
                  receivedParam,
                  debugConfig
                );
              }
              ...
              if (_.isArray(listenerQueue) && listenerQueue.length > 0) {
                listenerQueue.forEach(
                  (listenerConfig: TYPES.IListenerConfig) => {
                    const {
                      simpleMode: listenerMode,
                      listener
                    } = listenerConfig;
                    if (
                      listenerMode === true ||
                      _.isEqual(listenerMode, "true")
                    ) {
                      if (_.isFunction(listener)) {
                        listener(...args, defaultConfig);
                      }
              ...
              if (_.isArray(debugConfig) && debugConfig.length > 0) {
                eventRecord.finialInfo = this.generateDebugInfo(
                  receivedParam,
                  debugConfig
                );
              }
  ...
  getDynamicEventListener(event: TYPES.IEventConfig, simple?: boolean) {
    return (...args: any[]) => {
      if (_.isObject(event)) {
        const {
          eventName,
        ...
        if (
          ...
          // copy the received param
          const receivedParam = _.isObject(defaultParams)
            ? { ...defaultParams }
            : {};
  ...
  private mapEventRecords(indexes: number[]) {
    const history = this.history;
  ...
  private filterEventRecords(
    ...
    return records.filter((record?: TYPES.IEventRecord) => {
      if (_.isNil(record)) {
        ...
        if (
          excludeHasTarget &&
          (_.isString(targetName) || !_.isNil(targetRef))
        ) {
          return false;
        }
        ...
        if (
          includeTargetName.length > 0 &&
          _.isString(targetName) &&
          !includeTargetName.includes(targetName)
        ) {
          return false;
        }
        if (
          includeTargetRef.length > 0 &&
          !_.isNil(targetRef) &&
          !includeTargetRef.includes(targetRef)
        ) {
          return false;
        }
        if (
          includeEventName.length > 0 &&
          !includeEventName.includes(eventName)
        ) {
          return false;
        }
      }
  ...
  private mapListenerRecords(
    ...
    const indexOffset = history.indexOffset;
    return indexes.map(
      (index: { eventIndex: number; listenerIndex: number }) => {
        if (_.isObject(index)) {
  ...
  private filterListenerRecords(
    ...
    return records.filter((record?: TYPES.IListenerRecord) => {
      if (_.isNil(record)) {
        ...
        if (
          includeListenerName.length > 0 &&
          !includeListenerName.includes(listenerName)
        ) {
          return false;
        }
      }
  ...
  resetHistory(capacity?: number) {
    this.history = {
      capacity:
        !_.isNil(capacity) && _.isFinite(capacity) && capacity >= 0
          ? capacity
          : 100,
      lastStartNumber: 0,
      lastStoreNumber: 0,
  ...
  setHistoryCapacity(capacity: number) {
    const history = this.history;
  ...
  searchHistoryRecords(
    ...
    const history = this.history;
    const {
      indexTree: { targetTree, eventTree }
    } = history;
    ...
    if (
      ...
      if (_.isString(eventName) && eventName) {
        ...
        if (!_.isNil(subNode)) {
          return this.filterEventRecords(
            this.mapEventRecords(subNode.indexes),
            exclude
          );
        }
      } else {
        return this.filterEventRecords(
          this.mapEventRecords(node.indexes),
          exclude
        );
      }
    } else if (!_.isNil(targetRef)) {
      ...
      Object.keys(targetTree).some((name: string) => {
        ...
        if (!_.isNil(reference) && reference === targetRef) {
          ...
          if (_.isString(eventName) && eventName) {
            const subNode = node.eventTree[eventName];
            if (!_.isNil(subNode)) {
              result = this.filterEventRecords(
                this.mapEventRecords(subNode.indexes),
                exclude
              );
            }
  ...
  exportHistoryRecords(options?: TYPES.IEventExportOption) {
    const history = this.history;
    const {
      records,
      indexTree: { targetTree, eventTree, listenerTree }
