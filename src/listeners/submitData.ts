...
import { Cache, Request, StepProcess, DataPool, DataEngine } from "../helpers";
...
export type ISubmitProcessStatus =
  ...
  | "COMPLETED"
  | "FAILED";
const SubmitProcessRunStatus = ["NORMAL", "HAS_ERROR", "HAS_FAILURE"];
const SubmitProcessEndStatus = ["TERMINATED", "COMPLETED", "FAILED"];
...
export type ISubmitTargetStatus =
  ...
  | "SUCCESS"
  | "FAILURE";
const SubmitTargetRunStatus = ["NORMAL", "HAS_ERROR"];
const SubmitTargetEndStatus = ["SUCCESS", "FAILURE"];
...
async function submitProcess(
  ...
  if (!_.isNil(process) && _.isObject(process)) {
    ...
    if (_.isArray(targets) && targets.length > 0) {
        ...
        const promises = targets.map(
          async (config: ISubmitTarget | ISubmitProcess, index: number) => {
            if (_.has(config, "targets")) {
              ...
              if (
                pRecord.status === "FAILED" ||
                pRecord.status === "TERMINATED"
              ) {
                if (!ignore) {
                ...
                if (_.isArray(processRecord.errorInfo)) {
                  processRecord.errorInfo.push(
                    ...
                    }" in the sub-process ${pConfig.key ||
                      ""} of the process ${process.key || ""}`
                  );
                }
              }
              return pRecord;
      ...
      if (_.isArray(processRecord.errorInfo)) {
        processRecord.errorInfo.push(
          "The targets of the submit process is undefined or empty"
        );
      }
    }
  } else {
    ...
    default:
      if (_.isArray(processRecord.errorInfo)) {
        processRecord.errorInfo.push(
          `Found unexpected status "${
            processRecord.status
          }" in the end of the process ${process.key || ""}`
        );
      }
      break;
  }
  ...
  if (_.isObject(options)) {
    ...
    if (status === "COMPLETED" && _.isFunction(onProcessCompleted)) {
      ...
      await onProcessFailed(_.cloneDeep(process), _.cloneDeep(processRecord));
...
async function submitTarget(
  ...
  if (_.isObject(submitData) && !_.isArray(submitData)) {
    ...
    if (_.isString(wrapPath) && wrapPath) {
      ...
      const defaultWrapper = _.trim(schemaStr.replace(":", "."), ".")
        .split(".")
        .pop();
    ...
    try {
      ...
      if (_.isArray(targetRecord.errorInfo)) {
        targetRecord.errorInfo.push(
          `Failed to submit target ${target.key || sourceStr}`
        );
      }
    ...
    if (_.isArray(targetRecord.errorInfo)) {
      targetRecord.errorInfo.push(
        `Bad format: The data of ${sourceStr} can\'t be submited.`
      );
    }
  }
    ...
    default:
      if (_.isArray(targetRecord.errorInfo)) {
        targetRecord.errorInfo.push(
          `Found unexpected status "${
            targetRecord.status
          }" in the end of the target ${target.key || target.dataSource}`
        );
      }
      break;
  }
  ...
  if (_.isObject(options)) {
    const { onTargetSuccess, onTargetFailure } = options;
    const { status } = targetRecord;

    if (status === "SUCCESS" && _.isFunction(onTargetSuccess)) {
      await onTargetSuccess(_.cloneDeep(target), _.cloneDeep(targetRecord));
    } else if (status === "FAILURE" && _.isFunction(onTargetFailure)) {
      await onTargetFailure(_.cloneDeep(target), _.cloneDeep(targetRecord));
    }
  }
